...
void draw() {
	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			if(playerHealth < PLAYER_MAX_HEALTH && isHit(cabbageX[i],cabbageY[i],SOIL_SIZE,SOIL_SIZE,playerX,playerY,SOIL_SIZE,SOIL_SIZE)){ // r1 bottom edge past r2 top
...
void drawTimerUI(){
	int sec=gameTimer/60%60,minut=gameTimer/60/60;
	String timeString = nf(minut,2)+':'+nf(sec,2); // Requirement #4: Get the mm:ss string using String convertFramesToTimeString(int frames)
	...
	if(minut>=2){
		timeTextColor = #00ffff;
	}else if (minut>=1) {
		timeTextColor = #ffffff;
	}else if (sec>=30) {
		timeTextColor = #ffcc00;
	}else if (sec>=10) {
		timeTextColor = #ff6600;
	}
...
void drawCaution(){								// Requirement #6
	int soliderIndex=getEnemyIndexByRow(playerRow+5);
	if(soliderIndex==-1){
		return;
	}

	image(caution, soldierX[soliderIndex], soldierY[soliderIndex]-SOIL_SIZE);
